    ...
    <div class="card overflow-hidden transition-all duration-300 hover:shadow-card-hover">
        ...
        <form action="{{ route('users.update', $userModel->id) }}" method="POST" class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $userModel->name) }}" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2" required>
                @error('name')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $userModel->email) }}" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2" required>
                    @error('email')
                        <p class="text-xs text-red-600 mt-1 font-mono">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="bg-surface-50 p-4 rounded border border-gray-200">
                <span class="block text-xs font-mono font-bold text-slate-500 mb-2">Ganti Password (Opsional)</span>
                <!-- Password Grid -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <!-- Password -->
                    <div>
                        <label for="password" class="block text-xs font-bold uppercase tracking-wider text-blue-200 mb-1">Password Baru</label>
                        <input type="password" name="password" id="password" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2">
                        @error('password')
                            <p class="text-xs text-red-600 mt-1 font-mono">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="password" name="password_confirmation" id="password_confirmation" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2">
                    </div>
                </div>
            </div>
