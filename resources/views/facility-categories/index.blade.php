...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 card p-4 transition-all duration-300 hover:shadow-card-hover">
        <form action="{{ route('facility-categories.index') }}" method="GET" class="flex items-center gap-3 flex-1">
            <div class="w-full sm:w-64 relative">
                <input type="text" name="search" placeholder="Cari nama kategori..." value="{{ $filters['search'] ?? '' }}" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 pl-8 py-2">
            ...
            @if(!empty($filters['search']))
                <a href="{{ route('facility-categories.index') }}" class="text-xs text-slate-400 hover:text-primary underline">Reset</a>
            @endif
        </form>

        <a href="{{ route('facility-categories.create') }}" class="inline-flex items-center justify-center bg-primary text-slate-800 text-xs font-bold tracking-wider uppercase px-4 py-2.5 rounded hover:bg-primary-dark transition-colors self-start md:self-auto">
            Tambah Kategori
        </a>
    </div>
    ...
    <div class="card overflow-hidden transition-all duration-300 hover:shadow-card-hover">
                    ...
                    @forelse($categories as $cat)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="py-3.5 px-4 text-right space-x-2">
                                <a href="{{ route('facility-categories.edit', $cat->id) }}" class="inline-block text-[11px] font-semibold px-2 py-1 rounded border border-slate-600/50 text-slate-500 hover:bg-slate-100 transition-colors">
                                    Edit
                                </a>
        ...
        @if($categories->hasPages())
            <div class="px-5 py-4 border-t border-gray-200 bg-slate-55">
                {{ $categories->links() }}
            </div>
        @endif
    </div>
