...
<div class="max-w-2xl mx-auto">
    <div class="bg-gradient-to-br from-slate-900/80 via-blue-900/40 to-yellow-600/10 backdrop-blur-md rounded-lg border border-slate-700/50 shadow-sm overflow-hidden transition-all duration-500 hover:border-yellow-500/30 hover:shadow-lg hover:shadow-yellow-500/5">
        <div class="px-6 py-4 border-b border-slate-700/50 bg-slate-900/40 flex justify-between items-center">
            <h3 class="text-sm font-bold text-yellow-400 uppercase font-mono tracking-wider">Formulir Kategori Fasilitas</h3>
            <a href="{{ route('facility-categories.index') }}" class="text-xs text-slate-400 hover:text-slate-200">
                &larr; Kembali
            </a>
        </div>

        <form action="{{ route('facility-categories.store') }}" method="POST" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" class="w-full text-xs rounded border-slate-350 focus:border-secondary focus:ring-secondary py-2">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-2xs text-red-650 mt-1 font-mono">{{ $message }}</p>
                @enderror
            </div>
