    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gradient-to-br from-slate-900/80 via-blue-900/40 to-yellow-600/10 backdrop-blur-md p-4 rounded-lg border border-slate-700/50 shadow-sm transition-all duration-500 hover:border-yellow-500/30 hover:shadow-lg hover:shadow-yellow-500/5">
        <form action="{{ route('damage-categories.index') }}" method="GET" class="flex items-center gap-3 flex-1">
            ...
            @if(!empty($filters['search']))
                <a href="{{ route('damage-categories.index') }}" class="text-xs text-slate-400 hover:text-slate-200 underline">Reset</a>
            @endif
        </form>
                    ...
                    @forelse($categories as $cat)
                        <tr class="hover:bg-slate-700/30 transition-colors">
                            ...
                            <td class="py-3.5 px-4 text-right space-x-2">
                                <a href="{{ route('damage-categories.edit', $cat->id) }}" class="inline-block text-3xs font-semibold px-2 py-1 rounded border border-slate-600/50 text-slate-300 hover:bg-slate-700/50 transition-colors">
                                    Edit
                                </a>
