    ...
    <div class="card p-4 transition-all duration-300 hover:shadow-card-hover">
        <form action="{{ route('work-orders.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
            <div class="w-full sm:w-64 relative">
                <input type="text" name="search" placeholder="Cari judul laporan..." value="{{ $filters['search'] ?? '' }}" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 pl-8 py-2">
                    ...
                    @forelse($workOrders as $wo)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="py-3.5 px-4">
                                ...
                                <span class="block text-[10px] font-mono text-slate-600 mt-0.5">{{ $wo->damageReport->facility->facility_name }}</span>
                            </td>
                            <td class="py-3.5 px-4 text-slate-800 font-semibold">{{ $wo->assignee->name }}</td>
                            <td class="py-3.5 px-4 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    ...
                                    <span class="font-mono text-[11px] font-bold text-slate-800">{{ $wo->progress_percentage }}%</span>
                                </div>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                <span class="inline-flex px-2 py-0.5 rounded text-[10px] font-bold tracking-wider uppercase text-white shadow-3xs
