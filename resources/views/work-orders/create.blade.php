...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden p-5 flex flex-col sm:flex-row gap-4">
        ...
        <div class="flex-1">
            ...
            <p class="text-2xs text-slate-500 line-clamp-1 mt-1 leading-normal">{{ $damageReport->description }}</p>
            <div class="mt-2.5 flex gap-4 text-3xs font-mono text-slate-500">
                <div>Fasilitas: <span class="font-bold text-slate-700">{{ $damageReport->facility->facility_name }}</span></div>
    ...
    <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
            <h3 class="text-sm font-bold text-slate-800 uppercase font-mono tracking-wider">Formulir Penugasan Kerja</h3>
            <a href="{{ route('damage-reports.show', $damageReport->id) }}" class="text-xs text-slate-500 hover:text-slate-800">
                &larr; Batal
            </a>
        </div>
        ...
        <form action="{{ route('work-orders.store') }}" method="POST" class="p-6 space-y-5">
            ...
            <!-- Assignee -->
            <div>
                <label for="assigned_to" class="block text-xs font-bold uppercase tracking-wider text-slate-650 mb-1">Pilih Petugas (Maintenance Team)</label>
            ...
            <!-- Due Date -->
            <div>
                <label for="due_date" class="block text-xs font-bold uppercase tracking-wider text-slate-650 mb-1">Batas Waktu Penyelesaian (Due Date)</label>
            ...
            <!-- Instruction Notes -->
            <div>
                <label for="notes" class="block text-xs font-bold uppercase tracking-wider text-slate-650 mb-1">Catatan Instruksi Perbaikan</label>
