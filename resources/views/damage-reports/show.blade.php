...
<div class="space-y-6">
    <!-- Action Top Bar -->
    <div class="bg-white p-4 rounded-lg border border-slate-205 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center space-x-3">
            <a href="{{ route('damage-reports.index') }}" class="text-xs text-slate-500 hover:text-slate-800 font-semibold">&larr; Kembali ke Daftar</a>
            <span class="text-slate-350">|</span>
            <span class="text-xs font-mono font-bold text-slate-600">{{ $report->report_number }}</span>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            @if($report->status->value === 'draft')
                @if(auth()->id() === $report->reporter_id || auth()->user()->isAdmin())
                    <a href="{{ route('damage-reports.edit', $report->id) }}" class="bg-slate-100 text-slate-700 text-xs font-bold tracking-wider uppercase px-4 py-2.5 rounded border hover:bg-slate-200 transition-colors">
                        Edit Draft
                    </a>
            ...
            <!-- If Reported: Supervisor/Admin can verify or reject -->
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                    <h3 class="text-sm font-bold text-slate-800 uppercase font-mono tracking-wider">Rincian Laporan Kerusakan</h3>
                ...
                <div class="p-6 space-y-6">
                    <div>
                        <h2 class="text-lg font-bold text-slate-900 leading-tight">{{ $report->title }}</h2>
                        <div class="mt-2.5 flex flex-wrap gap-4 text-xs font-mono text-slate-500">
                            <div>Pelapor: <span class="font-bold text-slate-700">{{ $report->reporter->name }}</span></div>
                            <div>Tanggal: <span class="font-bold text-slate-700">{{ $report->created_at->format('d M Y H:i') }}</span></div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-b border-slate-100 py-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div>
                            ...
                            <span class="text-4xs font-mono text-slate-500">{{ $report->facility->facility_code }}</span>
                        </div>
                        <div>
                    ...
                    <!-- Description -->
                    <div class="space-y-2">
                        <span class="block text-xs font-bold uppercase tracking-wider text-slate-650">Deskripsi Kondisi</span>
                        <p class="text-xs text-slate-700 leading-relaxed whitespace-pre-line bg-slate-50 p-4 rounded border border-slate-150">{{ $report->description }}</p>
                    </div>

                    <!-- Photo Gallery -->
                    <div class="space-y-2">
                        <span class="block text-xs font-bold uppercase tracking-wider text-slate-650">Dokumentasi Foto ({{ $report->photos->count() }})</span>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            @foreach($report->photos as $photo)
                                <div class="relative group h-40 rounded overflow-hidden border border-slate-200 cursor-pointer" onclick="viewFullImage('{{ asset('storage/' . $photo->photo_path) }}')">
            ...
            <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50">
                    <h3 class="text-sm font-bold text-slate-800 uppercase font-mono tracking-wider">Log Riwayat Status Laporan</h3>
                </div>
                <div class="p-6">
                    <div class="flow-root">
                                            ...
                                            <div class="flex-1 min-w-0 pt-1.5 flex justify-between space-x-4">
                                                <div>
                                                    <p class="text-xs text-slate-600">
                                                        Status diubah ke <span class="font-bold text-slate-905 uppercase text-2xs px-1 py-0.5 rounded bg-slate-100 border border-slate-200">{{ str_replace('_', ' ', $log->status->value) }}</span>
                                                    </p>
                                                    @if($log->notes)
                                                ...
                                                <div class="text-right text-3xs whitespace-nowrap text-slate-400 flex flex-col items-end">
                                                    ...
                                                    <span class="font-bold text-slate-500 mt-0.5">oleh {{ $log->changedBy->name }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            @if($report->latitude && $report->longitude)
                <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                    <div class="px-5 py-4 border-b border-slate-200 bg-slate-50">
                        <h3 class="text-xs font-bold text-slate-800 uppercase font-mono tracking-wider">Lokasi Koordinat GIS</h3>
                    </div>
                    <div class="h-64 relative z-0" id="detail-map"></div>
                    <div class="p-4 bg-slate-50 border-t border-slate-200 font-mono text-3xs text-slate-500 flex flex-col space-y-1">
...
<div id="verification-modal" class="hidden fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
    ...
    <!-- Modal panel -->
    <div class="relative bg-white rounded-lg max-w-md w-full border border-slate-200 shadow-lg overflow-hidden z-10">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50">
            <h3 class="text-sm font-bold text-slate-800 uppercase font-mono tracking-wider" id="modal-title">Verifikasi Laporan</h3>
        </div>
        <form action="{{ route('damage-reports.verify', $report->id) }}" method="POST" class="p-5 space-y-4">
            ...
            <div>
                <label for="notes" class="block text-xs font-bold uppercase tracking-wider text-slate-650 mb-1">Catatan / Keterangan</label>
            ...
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="closeVerificationModal()" class="bg-slate-150 text-slate-700 text-xs font-semibold px-4 py-2 rounded hover:bg-slate-200">
                    Batal
                </button>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const detailMap = L.map('detail-map').setView([{{ $report->latitude }}, {{ $report->longitude }}], 16);
