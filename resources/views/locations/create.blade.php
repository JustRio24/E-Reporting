    ...
    <div class="card overflow-hidden transition-all duration-300 hover:shadow-card-hover">
        ...
        <form action="{{ route('locations.store') }}" method="POST" class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2" placeholder="Contoh: Transfer Tower 2 (TT2)" required>
                @error('name')
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2" placeholder="Jelaskan mengenai detail letak atau fungsi area tersebut...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-red-600 mt-1 font-mono">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="latitude" id="latitude" value="{{ old('latitude') }}" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2 font-mono" placeholder="Contoh: -2.993412">
                    @error('latitude')
                        <p class="text-xs text-red-600 mt-1 font-mono">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="longitude" id="longitude" value="{{ old('longitude') }}" class="w-full text-xs rounded border-gray-200 focus:border-primary focus:ring-primary/20 py-2 font-mono" placeholder="Contoh: 104.730894">
                    @error('longitude')
                        <p class="text-xs text-red-600 mt-1 font-mono">{{ $message }}</p>
                    @enderror
                </div>
